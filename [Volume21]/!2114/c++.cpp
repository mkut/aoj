...
pair<int,int> solve(int* _map, bool* result, int H, int W, int y, int x, unsigned long long o)
{
	if(y == H)
	{
		for(int i = 0; i < 8; i++)
	...
	if(white_ok)
	{
		pair<int,int> next = solve(map_white[0], result, H, W, x+1 == W ? y+1 : y, x+1 == W ? 0 : x+1, o);
		ret.first = max(ret.first, next.first);
		ret.second = min(ret.second, next.second);
	}
	if(black_ok)
	{
		pair<int,int> next = solve(map_black[0], result, H, W, x+1 == W ? y+1 : y, x+1 == W ? 0 : x+1, o | (1ull<<(y*8+x)));
...
int main()
{
	...
	while(cin >> W >> H, W != 0 || H != 0)
	{
		...
		int _map[8][8] = {};
		pair<int,int> ans = solve(_map[0], result[0], H, W, 0, 0, 0);
		int _max = ans.first;
		int _min = ans.second;
		if(_max - _min >= 0)
		{
